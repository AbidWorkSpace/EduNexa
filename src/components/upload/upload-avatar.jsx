...
export function UploadAvatar({ sx, error, value, disabled, helperText, className, ...other }) {
  ...
  const renderPreview = () => {
    ...
    if (typeof value === 'string' && value.startsWith('items/') && !preview) {
      return (
        <Box
          sx={{
            width: 1,
        ...
        >
          <Iconify icon="solar:gallery-round-bold" width={40} sx={{ color: 'text.disabled' }} />
        </Box>
      );
    }
    if (!preview) return null;
    return <Image alt="Avatar" src={preview} sx={{ width: 1, height: 1, borderRadius: '50%' }} />;
  };
