...
import { DEFAULT_TOOLBAR, DEFAULT_FLEX_COLUMN_MIN_WIDTH } from './table-defaults';
...
export function normalizeColumns(columns) {
  ...
  columns.forEach((column, index) => {
    ...
    // Default minWidth for flex columns (no override if column already has minWidth) for mobile readability
    const hasFlex = column.flex !== undefined && column.flex !== null;
    const hasNoMinWidth = column.minWidth === undefined || column.minWidth === null;
    const normalizedColumn =
      hasFlex && hasNoMinWidth
        ? { ...column, minWidth: DEFAULT_FLEX_COLUMN_MIN_WIDTH }
        : column;
