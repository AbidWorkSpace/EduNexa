...
function inMediumViewportRange(width) {
  return width >= MED_MIN && width <= MED_MAX;
}

/** Collapsed state for a saved nav layout (not POS transient mini). */
function getCollapsedForSavedNavLayout(navLayout) {
  if (navLayout === 'mini') return true;
  if (navLayout === 'vertical') {
    if (typeof window === 'undefined') return false;
    return inMediumViewportRange(window.innerWidth);
  }
  return false;
}
...
export function SidebarProvider({ children }) {
  ...
  const wasPosScreenRef = useRef(false);

  const pathname = usePathname();
  const { effectiveNavLayout, navLayout: savedNavLayout, isPosScreen } = useEffectiveNavLayout();

  // Mini mode forces collapsed state (including transient POS override)
  ...
  // POS: force collapsed. Leaving POS: restore openness per saved layout preference.
  useEffect(() => {
    const justLeftPos = wasPosScreenRef.current && !isPosScreen;
    wasPosScreenRef.current = isPosScreen;
    ...
    if (justLeftPos) {
      ...
      setIsCollapsed(getCollapsedForSavedNavLayout(savedNavLayout));
    }
  }, [isPosScreen, isMiniMode, savedNavLayout]);
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && inMediumViewportRange(window.innerWidth) && !isMiniMode) {
      setIsCollapsed(true);
    }

    let prevInMedRange =
      typeof window !== 'undefined' && inMediumViewportRange(window.innerWidth);

    const handleResize = () => {
      const w = window.innerWidth;
      const nowInMedRange = inMediumViewportRange(w);
