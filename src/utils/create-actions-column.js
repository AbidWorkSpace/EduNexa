...
import { ACTIONS_DROPDOWN_THRESHOLD, ACTIONS_COLUMN_MOBILE_WIDTH } from 'src/components/custom-table/table-defaults';
...
function ConfirmationDialog({ open, onClose, onConfirm, title, message, confirmText, cancelText }) {
  return (
    <Dialog open={open} onClose={onClose}>
      ...
      <DialogActions>
        <Field.Button type="button" onClick={onClose}>
          {cancelText || 'Cancel'}
        </Field.Button>
        <Field.Button
          type="button"
...
/**
 * Mobile actions dropdown: single button that opens a menu with all actions (with labels).
 * Used when isMobile && visible action count > threshold.
 */
function ActionsDropdownCell({ params, actions }) {
        ...
        {actions.map((action) =>
          action.href && !action.onClick ? (
            <MenuItem
              ...
              onClick={handleClose}
            >
              <ListItemIcon sx={{ minWidth: 40, mr: 1 }}>{action.icon}</ListItemIcon>
...
export function createActionsColumn(actions = [], options = {}) {
  ...
  const {
    ...
    isMobile = false,
    useDropdownWhenMoreThan = ACTIONS_DROPDOWN_THRESHOLD,
  } = options;
  ...
  // On mobile we show dropdown when >2 actions (single button) or up to 2 icons; use smaller width so data columns get more space
  const calculateWidth = () => {
    if (widthOption !== undefined) {
      return widthOption; // Use explicit width if provided
    }
    if (isMobile) {
      return ACTIONS_COLUMN_MOBILE_WIDTH;
    }
    // Desktop: base padding (32px) + (number of actions * 48px per action)
  ...
  const columnConfig = {
    ...
    getActions: (params) => {
      ...
      const visibleActionDescriptors = [];
      ...
      sortedActions.forEach((action, index) => {
        ...
        // Collect descriptor for mobile dropdown (same visible actions)
        visibleActionDescriptors.push({
          id: action.id,
          label,
          icon,
          onClick: href ? undefined : handleActionClick,
      ...
      // On mobile, when more than threshold visible actions, show dropdown instead of icon row
      if (isMobile && visibleActionDescriptors.length > useDropdownWhenMoreThan) {
        return [
          <ActionsDropdownCell key="actions-dropdown" params={params} actions={visibleActionDescriptors} />,
        ];
      }
