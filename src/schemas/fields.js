...
/**
 * numberFromInput(opts) - opts: { emptyAs?, min?, max?, nonnegative?, positive?, int?, fieldLabel? }
 * Omit emptyAs for required fields (empty input fails). Use emptyAs: 0 for optional default zero.
 * When fieldLabel is set, min/max/invalid messages match Branch Configurations API format.
 */
export function numberFromInput(opts = {}) {
  ...
  let schema = zod.number({
    invalid_type_error: fieldLabel
      ? `${labelPrefix}value must be a valid number.`
      : undefined,
    required_error: fieldLabel ? `${labelPrefix}value is required.` : undefined,
  });
  ...
  if (min !== undefined) {
    schema = schema.min(
      min,
      fieldLabel ? `${labelPrefix}value must be at least ${min}.` : undefined
    );
  }
  if (max !== undefined) {
    schema = schema.max(
      max,
      fieldLabel ? `${labelPrefix}value must be at most ${max}.` : undefined
    );
  }
